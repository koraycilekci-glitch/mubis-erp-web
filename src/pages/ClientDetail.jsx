        ...
        <div className="space-y-6">
          ...
          <div className="bg-blue-950/40 rounded-2xl p-6 border border-blue-800/30">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                { id: 'vergiUser', passId: 'vergiPass', label: 'Dijital Vergi Dairesi', icon: '🏛️' },
                { id: 'sgkUser', passId: 'sgkPass', label: 'SGK / e-Bildirge', icon: '🏥' },
                { id: 'itoUser', passId: 'itoPass', label: 'Ticaret Odasi', icon: '🏢' },
                { id: 'earsivUser', passId: 'earsivPass', label: 'e-Arsiv / e-Fatura Portal', icon: '📄' },
              ].map((portal) => (
                <div key={portal.id} className="bg-blue-900/20 rounded-xl p-4 border border-blue-700/20">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="text-lg">{portal.icon}</span>
                    <span className="text-white font-medium text-sm">{portal.label}</span>
                  </div>
                  {isEditing ? (
                    <div className="space-y-2">
                      <input
                        type="text"
                        value={editData[portal.id] || ''}
                        onChange={(e) => setEditData({...editData, [portal.id]: e.target.value})}
                        placeholder="Kullanici Adi / VKN"
                        className="w-full bg-blue-900/30 border border-blue-700/50 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-yellow-400"
                      />
                      <div className="relative">
                        <input
                          type={showPasswords[portal.passId] ? 'text' : 'password'}
                          value={editData[portal.passId] || ''}
                          onChange={(e) => setEditData({...editData, [portal.passId]: e.target.value})}
                          placeholder="Sifre"
                          className="w-full bg-blue-900/30 border border-blue-700/50 rounded-lg px-3 py-2 pr-10 text-white text-sm focus:outline-none focus:border-yellow-400"
                        />
                        <button
                          type="button"
                          onClick={() => setShowPasswords({...showPasswords, [portal.passId]: !showPasswords[portal.passId]})}
                          className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-300"
                        >
                          {showPasswords[portal.passId] ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                        </button>
                      </div>
                    </div>
                  ) : (
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="text-gray-400 text-xs">Kullanici:</span>
                        <span className="text-gray-200 text-sm font-mono">{client[portal.id] || '-'}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-gray-400 text-xs">Sifre:</span>
                        <div className="flex items-center gap-1">
                          <span className="text-gray-200 text-sm font-mono">
                            {client[portal.passId]
                              ? (showPasswords[portal.passId] ? client[portal.passId] : '••••••••')
                              : '-'
                            }
                          </span>
                          {client[portal.passId] && (
                            ...
                            >
                              {showPasswords[portal.passId] ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                            </button>
                          )}
                        </div>
                      </div>
          ...
          <div className="bg-blue-950/40 rounded-2xl p-6 border border-blue-800/30">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {[
                ...
                { label: 'MERSİS', url: 'https://mersis.gtb.gov.tr/', icon: '🌐', color: 'from-indigo-500/20 to-indigo-600/20 border-indigo-500/30' },
              ].map((item, i) => (
                <a
                  key={i}
                  href={item.url}
          ...
          <div className="bg-blue-950/40 rounded-2xl p-6 border border-blue-800/30">
            <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
              <span className="text-lg">📄</span> e-Fatura / e-Arsiv Portallari
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {[
                { label: 'GIB e-Fatura', url: 'https://ebelge.gib.gov.tr/efabortalgiris.html', icon: '📄', color: 'from-blue-500/20 to-blue-600/20 border-blue-500/30' },
                { label: 'GIB e-Arsiv', url: 'https://earsivportal.efatura.gov.tr/', icon: '📂', color: 'from-green-500/20 to-green-600/20 border-green-500/30' },
                { label: 'e-Fatura Sorgula', url: 'https://ebelge.gib.gov.tr/efabortalgiris.html', icon: '🔍', color: 'from-purple-500/20 to-purple-600/20 border-purple-500/30' },
                ...
                { label: 'e-Irsaliye', url: 'https://ebelge.gib.gov.tr/eiabortalgiris.html', icon: '🚚', color: 'from-teal-500/20 to-teal-600/20 border-teal-500/30' },
                { label: 'e-SMM Portal', url: 'https://esmm.gib.gov.tr/', icon: '📝', color: 'from-cyan-500/20 to-cyan-600/20 border-cyan-500/30' },
              ].map((item, i) => (
                <a
                  key={i}
                  href={item.url}
